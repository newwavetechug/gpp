...
class Disposal_m extends MY_Model
{
    ...
    #	print_r($post); exit();
	    ...
	    if(strcmp($strategic_asset,'Yes') != 0){
			$date_of_approval = 	date('Y-m-d',strtotime($post['date_of_approval']));
			$strategic_asseta = 'Y';
	     }
	    else
	    $date_of_approval = '';
		...
		$date_of_aoapproval =date('Y-m-d',strtotime($post['date_of_aoapproval']));
