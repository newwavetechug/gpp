                    ...
                    switch($this->input->post('exception_type')){
                        ...
                        case 'ETT':
                            ...
                            if($this->session->userdata('isadmin')=='Y'){
                                ...
                                //if pde filer results by pde
                                $pde=$this->session->userdata('pdeid');
                                $data['report_heading'] = 'BID EXCEPTION REPORT | BIDS ON TIME';
            ...
            $start_year=substr($this->input->post('financial_year'),0,4);
                ...
                else
                {
                    ...
                    if(!$this->input->post('from_date')){
                        //default to previouse year

                        $from=(date('Y')-1).'-'.'06'.'-30';//Fiscal year: 1 July - 30 June
                    ...
                    $data['reporting_period'] = '<b>'.custom_date_format('d M, Y', $from ). '</b> &nbsp<i> to </i> <b>  &nbsp &nbsp' . custom_date_format('d M, Y', $to).'</b>';
                            ...
                            if($this->session->userdata('isadmin')=='Y'){
                                ...
                                // }
                                    $pdestring = '';
                                    if(!empty($_POST['pde'])){
                                     $pdestring = strtoupper(get_pde_info_by_id($pde,'title'));

                                    }
                                        $data['report_heading'] = ''.$pdestring.' TIMELINESS FOR CONTRACT COMPLETION ';
                                ...
                                //if pde filer results by pde
                                $pde=$this->session->userdata('pdeid');
                                $data['report_heading'] = 'TIMELINESS FOR CONTRACT COMPLETION ';
                        ...
                        case 'procurement_lead_time_report':
                            ...
                            $data['notes'] = 'Only Procurments whose contracts are <b>Awarded and Completed</b> contracts were considered';


                            $data['from']=$from;
                            $data['to']=$to;

                            $data['report_heading'] = ' PROCUREMENT LEAD TIME BY PROCUREMENT METHOD ';
                            ...
                            if($this->session->userdata('isadmin')=='Y'){
                                if($this->input->post('pde')){
                                    $data['results']=$this->contracts_m->get_procurement_lead_times($from,$to,$this->input->post('pde'));



                                }else  {
                                    # code...
                                    $data['results']=$this->contracts_m->get_procurement_lead_times($from,$to,$pde='');
                        ...
                        case 'contracts_completed_within_original_value':
                            ...
                            if($this->session->userdata('isadmin')=='Y'){
                                ...
                                //if pde filer results by pde
                                $pde=$this->session->userdata('pdeid');
                                $data['report_heading'] = 'CONTRACTS COMPLETED WITHIN ORIGINAL VALUE ';
                        ...
                        case 'average_number_of_bids_per_contract':
                            ...
                            if($this->session->userdata('isadmin')=='Y'){
                                ...
                                if($this->input->post('pde')){
                                    ...
                                    $data['report_heading'] = strtoupper(get_pde_info_by_id($this->input->post('pde'),'title')). ' AVERAGE NUMBER OF BIDS PER CONTRACT ';
                                ...
                                //if pde filer results by pde
                                $pde=$this->session->userdata('pdeid');
                                $data['report_heading'] = ' AVERAGE NUMBER OF BIDS PER CONTRACT ';
                ...
                else
                {
                    ...
                    switch($this->input->post('report_type')){
                        case 'quarter_1':
                            ...
                            if($this->session->userdata('isadmin')=='Y'){
                                if($this->input->post('pde')){
                                    $data['report_heading'] = 'DISPOSAL REPORTS IN THE FIRST QUARTER <small>: ' . get_pde_info_by_id($this->input->post('pde'), 'title') . '</small>';
    ...
    function suspended_providers()
    {
        ...
        if($this->input->post('generate_ppms')){
              ...
              $data['page_title'] = 'Best Evaluated Bids awarded to suspended providers '.$this->input->post('financial_year');
                    ...
                    $where = array(
                        'isactive' => 'Y',
                        'datereceived >=' => date('Y-m-d', strtotime($start_year)),
                        'datereceived <=' => date('Y-m-d', strtotime($end_year)),
                        'beb' => 'Y'
                    );
